...
|---|---|---|---|---|
| B1 | Opening a demo can overwrite the active library play | **Critical** | behavioral | required — Phase 4.4 / 6.1 |
...
**B1 and B2 are the two that remain, and they are the reason this triage still matters.** Both are behaviors an implementer would otherwise reproduce faithfully, because the matrix's standing rule is that the rebuild must be identical to the original. They are now recorded as approved corrections in the parity matrix and assigned to Phases 4.4 and 6.1.
