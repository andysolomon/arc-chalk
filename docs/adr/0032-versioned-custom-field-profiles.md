...
- Every stored length uses yards. `hashInsetYards` and `numberInsetYards` are measured inward from each sideline; feet from prototype or rule-book sources are converted only at import boundaries.
