...
- `docs/parity/` contains the original-versus-production editor comparison plus a Playwright harness that locks the original Editor, Demo, Present, and Print desktop modes and seven global editor overlays at 1440 × 960. Production is measured against the original's own goldens, and each recorded gap is a ratchet that may only be lowered. The Editor sits at 1.71%. Present and Print are first-measured at 17.96% and 0.84%: Present is the full-window field on #171717 with type scaled 1.25×, and Print is the letter-landscape sheet that Export → Print the field produces. Demo is first-measured at 2.12%: the original's five guided sequences, with a demo handoff treated as a new unsaved Play so it cannot rewrite the Play that was open (parity-matrix B1). Five of the seven overlays — the More menu, Export menu, Save/version menu, command palette, and shortcut reference — are built from the original's own item lists, ordering, and copy and sit at 1.65–1.73%, except the shortcut reference at 2.50% where History lists named snapshots rather than the original's 90-second autosave ring. History, Page, and Type are now built: page kinds change the markings without moving the Play, Coach/Player/Print type presets scale the words, and annotation layers drop independently. All seven overlays are built: the Formation and Defense browsers are measured at 1.98% and 1.57%, with the original's catalogues as domain data, its All / Favorites / Mine tab strips, a star on every card, and the footer that keeps the offense on the field as a set of the Coach's own. Remaining panels, selection states, additional viewports, behaviors, and outputs are incomplete.
...
2. Close remaining 4.1 shell gaps: field viewBox aspect, tool-rail glyphs below the text tool, status-bar spacing, Formation-card row drift, and dynamic `paletteCommands`.
