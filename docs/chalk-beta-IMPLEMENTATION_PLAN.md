...
- The first production editor-shell slice recreates the original header, modes, field-first workspace, tool rail, seeded `Stick — Thunder` field, inspector, timeline, and status treatment. Its visible Field Profile markings, players, routes, branches, ticks, and labels are now projected from the canonical yard-space `RenderScene`; the surrounding controls remain comparison scaffolding, not a claim that Phase 4 visual or behavioral parity is complete.
