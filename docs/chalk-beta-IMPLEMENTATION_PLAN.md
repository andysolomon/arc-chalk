...
- The first production editor-shell slice recreates the original header, modes, field-first workspace, tool rail, seeded `Stick — Thunder` field, inspector, timeline, and status treatment. Its visible Field Profile markings, players, routes, branches, ticks, and labels are now projected from the canonical yard-space `RenderScene`, and the field accepts unified pointer and keyboard input through one pure state machine — selection, marquee, landmark-snapped moves, delete, nudge, and route creation each commit exactly one undoable transaction.
- `docs/parity/` contains the original-versus-production editor comparison plus a Playwright harness that locks the original Editor, Demo, Present, and Print desktop modes and seven global editor overlays at 1440 × 960. Production is measured against the original's own goldens, and each recorded gap is a ratchet that may only be lowered. The Editor sits at 1.65%. The field is drawn in the original's own 1000×620 frame. Present and Print are at 8.99% and 0.80%: Present is the full-window field on #171717 with type scaled 1.25×, and Print is the letter-landscape sheet that Export → Print the field produces. Demo is at 2.05%: the original's five guided sequences, with a demo handoff treated as a new unsaved Play so it cannot rewrite the Play that was open (parity-matrix B1). Five of the seven overlays — the More menu, Export menu, Save/version menu, command palette, and shortcut reference — are built from the original's own item lists, ordering, and copy and sit at 1.60–1.67%, except the shortcut reference at 2.42% where History lists named snapshots rather than the original's 90-second autosave ring. History, Page, and Type are now built: page kinds change the markings without moving the Play, Coach/Player/Print type presets scale the words, and annotation layers drop independently. All seven overlays are built: the Formation and Defense browsers are measured at 2.01% and 1.59%, with the original's catalogues as domain data, its All / Favorites / Mine tab strips, a star on every card, and the footer that keeps the offense on the field as a set of the Coach's own. Remaining Editor gap is Finding #3's Player shift against goldens captured from the uncorrected original. Present's remaining gap is the variation line, which waits on Phase 6.
- Phase 2 is complete. The current schema-version-3 Play contract and versioned Playbook, Concept, Formation, Field Profile, Assignment, revision, Play envelope, Playbook envelope, and sanitized Share Publication contracts validate structural and referential invariants with Zod Mini. Required Unit remains separate from optional built-in or custom Play Type, Personnel Label, Concept, and Formation sources. The pure v1→v2→v3 migration chain preserves released documents, promotes legacy route prose into hybrid Assignments, and retains permanent fixtures for every released shape. Stable source revisions and explicit Formation slot bindings support previewed reapplication without live-template coupling. Offensive and defensive Playbook goldens, generated round trips, canonical SHA-256 properties, malformed-reference properties, exact double mirroring, bounded yard geometry, absolute integer-millisecond path evaluation, framework-free `RenderScene` construction, accessible SVG primitives, and deterministic football-aware snapping now exercise the complete domain/rendering core without React, IndexedDB, or network dependencies.
- Phase 3 is complete. The fixed version-1 Dexie schema is private behind a Chalk-owned typed repository that persists Playbooks, Concepts, Formations, current Plays, immutable revisions, durable sync mutations, conflicts, preferences, private image blobs, per-Play undo history, search projections, and thumbnail derivatives. A Chalk-owned vanilla Zustand EditorStore separates transient title drafting from committed documents, serializes local commits with advancing optimistic hash guards, exposes saving/saved/error/retry acknowledgement, and measures the full request-to-acknowledgement interval against the strict 50 ms budget. Every document change now runs through that one serialized queue as a semantic Play command, so bounded, device-local, hash-guarded undo and redo chain from the hash of the document the previous entry produced, ride inside the Play commit transaction, and survive closing and reopening the Play; a stale or unreplayable entry is quarantined rather than applied. Coach-named immutable versions, a restore expressed as ordinary undoable commands, a thirty-day Trash, interrupted-session detection, and persistent-storage and quota handling complete the durability story. Databases written by earlier releases upgrade their Plays forward on read and once at startup, and interrupted writes are proven to leave either the prior valid state or the complete new state. Encrypted Coach-controlled backups round-trip current work, Trash state, and immutable named versions onto another device without ever overwriting newer local work. Golden round trips, forced late-transaction rollback, command inverse, difference, and retention properties, 2,000-Play scale-invariance coverage with a full history attached, real browser IndexedDB saving, and close/reload undo durability now exercise the seam without exposing Dexie or React through EditorStore.
- Phase 4 is complete. The original visual shell is in production primitives: header, modes, rail glyphs, inspector, status bar, Formation and Defense browsers, command palette, shortcut reference, Demo/Tour, Present, Print, Page, Type, and the 132×82 navigator. Direct manipulation, football workflows, camera, keyboard, touch, Pencil, and phone read-only behavior all run through one yard-space machine and one hash-guarded commit per completed gesture. Pointer movement coalesces onto animation frames and patches only the SVG that moved, so a `pointermove` never clones the Play or rebuilds the React field (ADR 0002, ADR 0015, review finding #13). Chromium at 1440×960 classifies 60 FPS and p95 input-to-paint against the 50 ms Coach-device budget and asserts both numeric ceilings on a developer machine. Playwright WebKit/iPad uses the same input-to-paint budget and the commit-delta architecture proof; it does not gate a numeric frame p95, because it is not a 60 Hz panel. Shared CI runners use the 8× ceiling for Chromium frame p95 and for input-to-paint on both browsers, matching item 3.2. Physical ninth-generation iPad and five-year-old laptop evidence remains Phase 12.3 / 10.4. Remaining Editor gap is Finding #3's Player shift against goldens captured from the uncorrected original.
- Phase 5 is implemented: the same yard-space Play the editor draws now plans a snap-relative timeline (pre-snap motion is negative; snap is 0 ms) and evaluates every player from an absolute integer millisecond. Defaults come from role and route shape; the first Timing edit persists those resolved values. The playback clock never updates React every frame — rAF patches player transforms and ghost trails — and Editor and Present share one evaluator and the same 34px bar. Reduced motion jumps to the end. Space plays and pauses; a held space still pans. Frame-sequence PNGs plus a snap-relative manifest, and a four-frame progression strip, consume the same evaluator. GIF and MP4 stay out of scope. Share Links import those same functions in the static `/s/{publicId}` shell.
- Phase 8 is implemented as a production extension: images are validated, oriented, EXIF-stripped, resized, thumbnailed, and hashed on the device (Worker + OffscreenCanvas where available) before IndexedDB; private R2 uses content-addressed keys and short-lived SigV4 URLs from authenticated Convex functions; Share Publications are immutable sanitized snapshots; `/s/{publicId}#{secret}` fragment-capability links store only keyed hashes, with revoke/republish/expiry and access audits; the static share shell has a strict CSP, `Referrer-Policy: no-referrer`, no third-party scripts, and read-only playback; Film References are https display links that are never proxied or cached. Phase 7 auth is still open, so cloud publish/R2 require a configured Convex deployment and an authenticated Coach; local attach and film remain offline.
...
2. Phases 4, 5, 6, 7, 8, 9, and the implementable scope of 10 are closed: the original inspector library is back with an additive virtualized Playbook browser, worker search, and revision-keyed thumbnails; Clerk auth and the conflict-safe Convex replica are in; private assets and Share Links are in; every original Export action runs; the app-shell-only service worker and lifecycle UX are in.
3. Remaining Editor gap is Finding #3's Player shift against goldens captured from the uncorrected original. Next production slices are the Phase 10 device matrix and startup budgets, then Phase 11 operations.
4. Record only explicit product-owner-approved parity exceptions; implementation convenience is not an exception.
5. Close each parity group end-to-end with Coach-visible tests and original-versus-production evidence, keeping `docs/chalk-beta-progress.txt` synchronized as each lands.
