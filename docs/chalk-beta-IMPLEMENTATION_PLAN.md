...
- The first production editor-shell slice recreates the original header, modes, field-first workspace, tool rail, seeded `Stick — Thunder` field, inspector, timeline, and status treatment. Its visible Field Profile markings, players, routes, branches, ticks, and labels are now projected from the canonical yard-space `RenderScene`, and the field accepts unified pointer and keyboard input through one pure state machine — selection, marquee, landmark-snapped moves, delete, and nudge each commit exactly one undoable transaction. The surrounding controls remain comparison scaffolding, not a claim that Phase 4 visual or behavioral parity is complete.
