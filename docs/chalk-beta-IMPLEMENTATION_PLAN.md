...
- Phase 4.6 is implemented: pointer movement coalesces onto animation frames and patches only the SVG that moved, so a `pointermove` never clones the Play or rebuilds the React field (ADR 0002, ADR 0015, review finding #13). Chromium at 1440×960 and WebKit/iPad classify 60 FPS and p95 input-to-paint against the 50 ms Coach-device budget, with the same 8× CI ceiling item 3.2 uses. Physical ninth-generation iPad and five-year-old laptop evidence remains Phase 12.3 / 10.4.
