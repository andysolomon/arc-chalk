...
- The Phase 2 tracers extract the original runtime's seeded `Stick — Thunder` Play into a strict Zod Mini schema-version-2 production document, migrate its 12-units-per-yard canvas into centered LOS-relative yard space, and prove canonical serialization/SHA-256, exact double mirroring, deterministic framework-free `RenderScene` construction, and SVG viewport projection used by the production shell. Versioned NFHS, NCAA, NFL, and custom Field Profiles use explicit yard units, generate bounded landmarks, and migrate the released ambiguous v1 hash property. Structured yard-space paths now cover every original path kind, line style, ending, curve, segment override, choice branch, alternate, block tick, and sized coverage area; their bounded quadratic sampler drives deterministic integer-millisecond distance evaluation. The remaining Phase 2 Coach-owned schemas, player and label primitives, smart snapping, and broader golden coverage are still open.
...
2. Continue Phase 2 by extending the canonical schemas and `RenderScene` projection across the original's remaining player, label, and field annotation primitives.
3. Use the completed structured path geometry and Field Profile landmarks for deterministic smart snapping without changing the Coach-facing appearance.
