...
- The Phase 2 tracers extract the original runtime's seeded `Stick — Thunder` Play into a strict Zod Mini schema-version-2 production document, migrate its 12-units-per-yard canvas into centered LOS-relative yard space, and prove canonical serialization/SHA-256, exact double mirroring, deterministic framework-free `RenderScene` construction, and SVG viewport projection used by the production shell. Versioned NFHS, NCAA, NFL, and custom Field Profiles use explicit yard units, generate bounded landmarks, and migrate the released ambiguous v1 hash property. Structured yard-space paths cover every original path kind, line style, ending, curve, segment override, choice branch, alternate, block tick, and sized coverage area; their bounded quadratic sampler drives deterministic integer-millisecond distance evaluation. Player and label primitives preserve every original symbol, fill, side/unit, group, role, color, label, sublabel, box, typography treatment, bound position, and leader style behind the same render seam. The pure editor snapping module now ranks ball/LOS, Field Profile landmarks, nearby player/route-node alignment, equal splits, and configurable yard grids with an eight-screen-pixel threshold; returns visible guide data and group-preserving yard deltas; and computes route constraints at grass-true 45-degree increments. The remaining Phase 2 Coach-owned schemas and broader golden coverage are still open.
...
2. Extend the remaining Coach-owned schemas and golden coverage only through complete vertical parity slices.
3. Begin the Phase 3 local database seam only after its required Phase 2 schemas and migration fixtures are defined.
