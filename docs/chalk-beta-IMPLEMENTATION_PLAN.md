...
- `docs/parity/` contains the original-versus-production editor comparison plus a Playwright harness that locks the original Editor, Demo, Present, and Print desktop modes and seven global editor overlays at 1440 × 960. Production is measured against the original's own goldens, and each recorded gap is a ratchet that may only be lowered. The Editor sits at 1.85%. Five of the seven overlays — the More menu, Export menu, Save/version menu, command palette, and shortcut reference — are now built from the original's own item lists, ordering, and copy and are measured at 1.74–1.81%, except the shortcut reference at 2.64% where the original's inspector carries History, Page, and Type sections production has not built. All seven overlays are now built: the Formation and Defense browsers are measured at 1.98% and 1.57%, with the original's catalogues as domain data, its All / Favorites / Mine tab strips, a star on every card, and the footer that keeps the offense on the field as a set of the Coach's own. Remaining panels, selection states, additional viewports, behaviors, and outputs are incomplete.
...
2. Implement bounded hash-guarded semantic undo/redo in the same local transaction as each completed editor gesture.
3. Add explicit named versions, 30-day Trash, startup recovery, quota monitoring, and persistent-storage UX without weakening continuous local save.
4. Preserve the fixed version-1 IndexedDB fixture when the first schema upgrade is introduced, then prove real older-database migration and interrupted-upgrade recovery.
5. Record only explicit product-owner-approved parity exceptions; implementation convenience is not an exception.
6. Close each parity group end-to-end with Coach-visible tests and original-versus-production evidence, keeping `docs/chalk-beta-progress.txt` synchronized as each executable slice lands.
