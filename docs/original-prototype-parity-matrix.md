...
- Editable Play title, category treatment (now document-bound Unit · Type — ADR 0041), undo/redo, More, Export, and Save/version menus.
- Full-window field-first workspace with independently collapsible tool rail and inspector, Focus mode, persistent chrome state, status bar, and contextual help (the inspector's idle order and folding are the ADR 0043 exception).
